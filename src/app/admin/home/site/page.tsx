...
export default function AdminHomeSitePage() {
  ...
  const addNavLink = async () => {
    ...
    const r = await fetch("/api/home/nav-links", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const updateNavLinkHref = async (id: string, href: string) => {
    await fetch(`/api/home/nav-links/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  const deleteNavLink = async (id: string) => {
    await fetch(`/api/home/nav-links/${id}`, { method: "DELETE" });
