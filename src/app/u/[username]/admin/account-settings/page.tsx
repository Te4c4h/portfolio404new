...
export default function AccountSettingsPage() {
  ...
  if (!profile) return <div className="text-[#888] text-sm">Could not load profile.</div>;

  return (
    <div className="max-w-3xl">
      <h1 className="text-2xl font-bold text-[#fafafa] mb-6">Account Settings</h1>

      <div className="space-y-6">
        {/* Profile */}
        <div className="bg-[#181818] border border-[#2a2a2a] rounded-xl p-5">
          <h2 className="text-xs font-semibold text-[#888] uppercase tracking-wider mb-4">Profile</h2>
          ...
          <button onClick={handleSaveProfile} disabled={savingProfile} className="px-4 py-2 rounded-lg text-sm font-medium bg-[#70E844] text-[#131313] hover:bg-[#5ed636] disabled:opacity-50">
        ...
        <div className="bg-[#181818] border border-[#2a2a2a] rounded-xl p-5">
          <h2 className="text-xs font-semibold text-[#888] uppercase tracking-wider mb-4">Username & Portfolio URL</h2>
          <p className="text-xs text-[#FE454E]/80 mb-3">⚠ Changing your username will change your portfolio URL.</p>
          <div className="mb-3">
            ...
            {usernameStatus === "available" && (
              <p className="text-[#70E844] text-xs mt-1">Username is available</p>
            )}
            <p className="text-[#555] text-xs mt-1">
              Portfolio URL: <span className="text-[#888]">portfolio404.site/u/{newUsername.trim().toLowerCase() || currentUsername}</span>
            </p>
          </div>
          {usernameError && <p className="text-[#FE454E] text-sm mb-3">{usernameError}</p>}
          <button
            onClick={handleSaveUsername}
            disabled={savingUsername || usernameStatus !== "available"}
            className="px-4 py-2 rounded-lg text-sm font-medium bg-[#70E844] text-[#131313] hover:bg-[#5ed636] disabled:opacity-50"
          >
        ...
        {/* Change Password */}
        <div className="bg-[#181818] border border-[#2a2a2a] rounded-xl p-5">
          <h2 className="text-xs font-semibold text-[#888] uppercase tracking-wider mb-4">Change Password</h2>
          {profile.isOAuth ? (
            <p className="text-[#888] text-sm">Your account uses Google sign-in. Password change is not available.</p>
          ) : (
            <>
              <div className="space-y-3 mb-4">
                <div>
                  <label className="text-xs text-[#888] mb-1 block">Current Password</label>
              ...
              {passwordError && <p className="text-[#FE454E] text-sm mb-3">{passwordError}</p>}
              {passwordSuccess && <p className="text-[#70E844] text-sm mb-3">Password changed successfully!</p>}
              <button onClick={handleChangePassword} disabled={savingPassword} className="px-4 py-2 rounded-lg text-sm font-medium bg-[#70E844] text-[#131313] hover:bg-[#5ed636] disabled:opacity-50">
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setDeleteModalOpen(false)}>
          <div className="bg-[#181818] border border-[#2a2a2a] rounded-xl p-6 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-[#888] text-sm mb-2">
              Type <strong className="text-[#FE454E]">{profile.username}</strong> to confirm:
            </p>
            <input
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => { setDeleteModalOpen(false); setDeleteConfirm(""); }} className="px-4 py-2 rounded-lg text-sm bg-[#2a2a2a] text-[#fafafa] hover:bg-[#333]">
                Cancel
              </button>
              <button
